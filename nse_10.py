...
#building on nse_8 video
#reading financials-historical-ratio
...
try:
    for item in df['link']:
        ...
        if company!='':#'coal-india-ltd_coalindia':
            ...
            stock_index_file_in=Path(stock_path +'\\'+ company +'\\financials-historical-ratio')
...
##                    for count in counts:
                    ...
                    if count==len(tr_total)-1:
                        ...
                        new_df1=df1.filter(['stock_ticker','Year','ROA(%)','ROE(%)','ROCE(%)','PER(x)',
                                            'PCE(x)','Price / Book(x)','Yield(%)','M Cap / Sales',
                                            'Total Debt / Equity(x)','Interest'],axis=1)
...
##                        df1[]
       ...
       # time.sleep(1)
    result_df.to_csv('yearly-ratios.csv')
